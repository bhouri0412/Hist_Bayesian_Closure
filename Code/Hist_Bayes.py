...
class ODEfit_HMC:
    ...
    def loss(self, params, batch_Xt, batch_Xtpdt, idx):
        ...
        # transfer learning
#        batch_Xt = np.concatenate((batch_Xt,pred[:,None,:]),axis=1)
#        for iloc in range(n_fut_reinf-1):
#            pred = stepper(batch_Xt, params, self.num_par_NN, self.K, self.L, self.dt, self.sigma_X, self.mu_X, F)
#            batch_Xt = np.concatenate((batch_Xt,pred[:,None,:]),axis=1)
#        loss = np.mean((pred - batch_Xtpdt)**2)
        return loss
...
# Target  state at the next time-step
model_HMC = ODEfit_HMC(Xt, Xtpdt, L, dt, sigma_X, mu_X)
# Target state at the next n_fut_transfer-th time-step
#model_HMC = ODEfit_HMC(Xt_transfer, Xtpdt_reinf, L, dt, sigma_X, mu_X)
...
epsilon = 0.0001 # leap frog step size
epsilon_max = epsilon
epsilon_min = epsilon
...
forward_pass_v = vmap(forward_pass, in_axes = (None, None, None, 0, None, None, None, None, None))
...
if compute_results_Bayes == 1:
    ...
    Xpred_int = integrate_L96_2t_with_NN_v(X_int[0:2*n_hist+2,:], si, nt-2*n_hist-1, parameters, model_transfer, F, 0, 2*dt)
    ...
    NN_int = forward_pass_v(Xt_dt, Xth1_dt, Xth2_dt, parameters, model_transfer.num_par_NN, model_transfer.K, model_transfer.L, model_transfer.sigma_X, model_transfer.mu_X) 
    ...
    Xpred_ext = integrate_L96_2t_with_NN_v(Xpred_init, si, nt, parameters, model_transfer, F, 0, 2*dt)
    Xpred_ext = Xpred_ext[:,2*n_hist+1:,:]
    ...
    NN_ext = forward_pass_v(Xt_dt, Xth1_dt, Xth2_dt, parameters, model_transfer.num_par_NN, model_transfer.K, model_transfer.L, model_transfer.sigma_X, model_transfer.mu_X) 
