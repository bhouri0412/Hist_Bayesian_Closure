...
if is_plot_interp == 1:
    ...
    for kk in range(4):
        for jj in range(2):
            ...
            plt.plot(t_2dt, NN_int[0,:,ii], '-', color = "gray", alpha = 0.8, label = "Sample Trajectory from posterior", linewidth = 0.5)
            plt.plot(t_2dt, NN_int[:,:,ii].T, '-', color = "gray", alpha = 0.8, linewidth = 0.5)
    ...
    plt.savefig(fold_save+'/interp_non_hist_Bayes_samples_closure.png', dpi = 300, bbox_inches='tight',pad_inches = 0.1)
    plt.close()
    ...
    for kk in range(4):
        for jj in range(2):
            ...
            plt.yticks(fontsize=22)
            plt.plot(t_2dt, exact_out, 'r--', label = "True")
            plt.plot(t_2dt, NN_mu[:,ii], 'b-', label = "Mean prediction")
            lower_0 = NN_mu[:,ii] - 2.0*np.sqrt(NN_Sigma[:,ii])
            upper_0 = NN_mu[:,ii] + 2.0*np.sqrt(NN_Sigma[:,ii])
            plt.fill_between(t_2dt.flatten(), lower_0.flatten(), upper_0.flatten(), 
    ...
    plt.savefig(fold_save+'/interp_non_hist_Bayes_mean_closure.png', dpi = 300, bbox_inches='tight',pad_inches = 0.1)
    plt.close()
    ...
    plt.savefig(fold_save+'/interp_non_hist_Bayes_samples_X.png', dpi = 300, bbox_inches='tight',pad_inches = 0.1) 
    plt.close()
    ...
    plt.savefig(fold_save+'/interp_non_hist_Bayes_mean_X.png', dpi = 300, bbox_inches='tight',pad_inches = 0.1)
    plt.close()
...
if is_plot_extrap == 1:
    ...
    for kk in range(4):
        for jj in range(2):
            ...
            plt.xticks(fontsize=22)
            plt.yticks(fontsize=22)
            plt.plot(t_2dt_ext, NN_ext[0,:,ii], '-', color = "gray", alpha = 0.8, label = "Sample Trajectory from posterior", linewidth = 0.5)
            plt.plot(t_2dt_ext, NN_ext[:,:,ii].T, '-', color = "gray", alpha = 0.8, linewidth = 0.5)
    ...
    plt.savefig(fold_save+'/non_hist_Bayes_samples_closure.png', dpi = 300, bbox_inches='tight',pad_inches = 0.1)
    plt.close()
    ...
    for kk in range(4):
        for jj in range(2):
            ...
            plt.yticks(fontsize=22)
            plt.plot(t_2dt_ext, exact_out, 'r--', label = "True")
            plt.plot(t_2dt_ext, NN_MAP_ext[:,ii], 'b-', label = "MAP")
            lower_0 = NN_mu_ext[:,ii] - 2.0*np.sqrt(NN_Sigma_ext[:,ii])
            upper_0 = NN_mu_ext[:,ii] + 2.0*np.sqrt(NN_Sigma_ext[:,ii])
            plt.fill_between(t_2dt_ext.flatten(), lower_0.flatten(), upper_0.flatten(), 
                             facecolor='orange', alpha=0.5, label="Two std band")
            plt.xlabel('$t$ (ATM days)',fontsize=26)
            plt.ylabel(r'Closure for $X_'+str(ii+1)+'$',fontsize=26)
            if kk==0 and jj==0:
    ...
    plt.savefig(fold_save+'/non_hist_Bayes_mean_closure.png', dpi = 300, bbox_inches='tight',pad_inches = 0.1)   
    plt.close()
    ...
    plt.savefig(fold_save+'/non_hist_Bayes_samples_X.png', dpi = 300, bbox_inches='tight',pad_inches = 0.1)   
    plt.close()
    ...
    plt.savefig(fold_save+'/non_hist_Bayes_mean_X.png', dpi = 300, bbox_inches='tight',pad_inches = 0.1)
    plt.close()
...
rel_unc = np.mean( Sigma_pred/ (np.abs(X_int)) )
rel_unc_ext = np.mean( Sigma_pred_ext/ (np.abs(X_ext)) )
print('Relative interpolation uncertainty for X: ',rel_unc)
print('Relative extrapolation uncertainty for X: ',rel_unc_ext)
...
rel_unc = np.mean( NN_Sigma.T/ (np.abs(exact_out_int)) )
rel_unc_ext = np.mean( NN_Sigma_ext.T/ (np.abs(exact_out_ext)) )
print('Relative interpolation uncertainty for Closure: ',rel_unc)
print('Relative extrapolation uncertainty for Closure: ',rel_unc_ext)